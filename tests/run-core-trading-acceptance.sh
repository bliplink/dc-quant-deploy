...
if [[ "${ORDER_CLUSTER_ENABLED:-false}" == "true" ]]; then
  expected_memory="$(memory_limit_bytes "${ORDERSVR_B_MEMORY_LIMIT:-3072m}")"
  ...
  grep -Fq -- '-Xmx2048m' <<<"${java_command}" ||
    die "Effective JVM heap for dc-saas-ordersvr-b is not -Xmx2048m: ${java_command}"
  (( memory_bytes == expected_memory )) ||
    die "Memory limit for dc-saas-ordersvr-b is ${memory_bytes}, expected ${expected_memory}"
  printf '[core-acceptance] MEMORY %s limit_bytes=%s effective_xmx=%s\n' \
    dc-saas-ordersvr-b "${memory_bytes}" 2048m
fi

if [[ "${ORDER_CLUSTER_C_ENABLED:-false}" == "true" ]]; then
  expected_memory="$(memory_limit_bytes "${ORDERSVR_C_MEMORY_LIMIT:-3072m}")"
  java_command="$(docker exec dc-saas-ordersvr-c sh -c "ps -ef | grep '[j]ava' | head -n 1")"
  memory_bytes="$(docker inspect --format '{{.HostConfig.Memory}}' dc-saas-ordersvr-c)"
  grep -Fq -- '-Xmx2048m' <<<"${java_command}" ||
    die "Effective JVM heap for dc-saas-ordersvr-c is not -Xmx2048m: ${java_command}"
  (( memory_bytes == expected_memory )) ||
    die "Memory limit for dc-saas-ordersvr-c is ${memory_bytes}, expected ${expected_memory}"
  printf '[core-acceptance] MEMORY %s limit_bytes=%s effective_xmx=%s\n' \
    dc-saas-ordersvr-c "${memory_bytes}" 2048m
fi
