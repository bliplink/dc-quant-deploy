...
write_hourly_summary() {
  local hour last_hour db_stats web_stats status
  local db_samples min_bid max_bid min_ask max_ask http_bad accepted_delta rejected_delta gap_delta auth_delta
  ...
  IFS=$'\t' read -r db_samples min_bid max_bid min_ask max_ask http_bad accepted_delta rejected_delta gap_delta auth_delta <<<"${db_stats}"
  ...
  status=PASS
  if (( db_samples == 0 && web_minutes == 0 )); then
    status=NO_DATA
  else
    if (( db_samples == 0 || min_bid < 10 || min_ask < 10 || http_bad > 0 || rejected_delta > 0 || gap_delta > 0 )); then status=FAIL; fi
  ...
  printf '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n' \
    "${hour}:00Z" "${db_samples}" "${min_bid}" "${max_bid}" "${min_ask}" "${max_ask}" \
...
(( gap_delta == 0 )) || issues+=("database_gap_delta_${gap_delta}")
...
d={"time":timestamp,"healthy":issue_count=="0","loadPid":integer(load_pid),
   ...
   "databaseDepth":{"bids":integer(bids),"asks":integer(asks)},"webHttp":integer(http),
...
: >"${LAST_ALERT_FILE}"
log "HEALTHY robot=${robot_status}/${robot_orders} db=${bid_levels}+${ask_levels} web=${web_min_bid}-${web_max_bid}+${web_min_ask}-${web_max_ask} accepted=${accepted} rejected=${rejected} auth_refresh_delta=${auth_delta}"
