...
for container in dc-saas-cluster-ordersvr-a dc-saas-cluster-ordersvr-b dc-saas-cluster-gateway; do
...
wait_gateway_node() {
  ...
  while (( SECONDS < deadline )); do
    if sudo docker logs --since "${STARTED_AT}" dc-saas-cluster-gateway 2>&1 |
...
sleep 2
sudo docker logs --since "${STARTED_AT}" dc-saas-cluster-gateway >"/tmp/order-cluster-gw-${RUN_ID}.log" 2>&1
sudo docker logs --since "${STARTED_AT}" dc-saas-cluster-ordersvr-a >"/tmp/order-cluster-a-${RUN_ID}.log" 2>&1
sudo docker logs --since "${STARTED_AT}" dc-saas-cluster-ordersvr-b >"/tmp/order-cluster-b-${RUN_ID}.log" 2>&1
