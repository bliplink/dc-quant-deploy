...
(
    ...
    `open` Nullable(Decimal(76, 9)) CODEC(LZ4),
    `high` Nullable(Decimal(76, 9)) CODEC(LZ4),
    `low` Nullable(Decimal(76, 9)) CODEC(LZ4),
    `close` Nullable(Decimal(76, 9)) CODEC(LZ4),
    ...
    `openInterest` Nullable(Decimal(76, 9)) CODEC(LZ4),
    `preSettlePrice` Nullable(Decimal(76, 9)) CODEC(LZ4),
    `turnover` Decimal(76, 2) CODEC(LZ4),
    `volume` Decimal(76, 2) CODEC(LZ4)
)
ENGINE = ReplacingMergeTree(createTime)
PARTITION BY toYYYYMM(startTime)
PRIMARY KEY (startTime, securityID, text, fmtTime)
ORDER BY (startTime, securityID, text, fmtTime)
