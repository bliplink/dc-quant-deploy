...
TRADE_CLUSTER_PERIODIC_SNAPSHOT_ENABLED="${TRADE_CLUSTER_PERIODIC_SNAPSHOT_ENABLED:-false}"
TRADE_CLUSTER_PERIODIC_SNAPSHOT_POLL_MILLIS="${TRADE_CLUSTER_PERIODIC_SNAPSHOT_POLL_MILLIS:-5000}"
TRADE_CLUSTER_PERIODIC_SNAPSHOT_MAX_AGE_MILLIS="${TRADE_CLUSTER_PERIODIC_SNAPSHOT_MAX_AGE_MILLIS:-300000}"
TRADE_CLUSTER_PERIODIC_SNAPSHOT_MIN_COMMITTED_MUTATIONS="${TRADE_CLUSTER_PERIODIC_SNAPSHOT_MIN_COMMITTED_MUTATIONS:-10000}"
PROTO_VERSION=1
...
if [[ "${TRADE_CLUSTER_ENABLED}" == "true" ]]; then
  ...
  [[ "${TRADE_CLUSTER_PERIODIC_SNAPSHOT_ENABLED}" == "true" || "${TRADE_CLUSTER_PERIODIC_SNAPSHOT_ENABLED}" == "false" ]] ||
    die "TRADE_CLUSTER_PERIODIC_SNAPSHOT_ENABLED must be true or false"
  for name in TRADE_CLUSTER_PERIODIC_SNAPSHOT_POLL_MILLIS TRADE_CLUSTER_PERIODIC_SNAPSHOT_MAX_AGE_MILLIS \
      TRADE_CLUSTER_PERIODIC_SNAPSHOT_MIN_COMMITTED_MUTATIONS; do
    [[ "${!name}" =~ ^[1-9][0-9]*$ ]] || die "${name} must be a positive integer"
  done
fi
...
trade.cluster.snapshot.periodic.enabled=${TRADE_CLUSTER_PERIODIC_SNAPSHOT_ENABLED}
trade.cluster.snapshot.periodic.pollMillis=${TRADE_CLUSTER_PERIODIC_SNAPSHOT_POLL_MILLIS}
trade.cluster.snapshot.periodic.maxAgeMillis=${TRADE_CLUSTER_PERIODIC_SNAPSHOT_MAX_AGE_MILLIS}
trade.cluster.snapshot.periodic.minCommittedMutations=${TRADE_CLUSTER_PERIODIC_SNAPSHOT_MIN_COMMITTED_MUTATIONS}
...
trade.cluster.replication.degradedRetryMillis=${TRADE_CLUSTER_REPLICATION_DEGRADED_RETRY_MILLIS:-1000}
trade.cluster.replication.bindHost=127.0.0.1
trade.cluster.replication.port=${replication_port:-19092}
trade.cluster.replication.requestTimeoutMs=${TRADE_CLUSTER_REPLICATION_REQUEST_TIMEOUT_MS:-1000}
