...
本报告第 4 节是 2026-08-31 阶段性窗口，保留用于说明缺档问题的发现过程。之后又完成两项资源修复：GW `6702148` 将 HTTP 回调工作组改为全连接共享；`gateway-api` 3.0.2 将 TradeSvr 入站消息改为 32 个固定 worker、10,000 有界队列和调用方背压。

最终 305.3 秒无故障窗口新增接受 416、拒绝 0、数据库缺档 0；真实 Chromium 新增 1,188 个采样，买卖盘最小/最大均为 10/10，缺档 0、页面错误 0。APSSvr 停止后 Robot 4 秒内进入 `STALE/0` 并安全撤单，服务恢复后无需重启 GW/RobotSvr，约 47 秒自动回到 `RUNNING/20`；随后 141 秒窗口新增 544 个严格 10+10 Web 样本且无错误。

3000×32 压力下，TradeSvr 从修复前约 4,381 个线程、707 MiB 收敛为 169–170 PID（其中 32 个 gateway-api worker）、259–279 MiB，9,001 个 API 操作和全部业务核对继续通过。最终报告和原始 JSON 见 `docs/CORE_TRADING_STABILITY_AND_STRESS_ACCEPTANCE_20260901.zh-CN.md` 与 `docs/evidence/`。
