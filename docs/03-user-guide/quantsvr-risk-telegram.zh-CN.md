...
`QuantSvr` 是实盘运行服务，也可以理解为 `EMS + Risk + Telegram` 的组合：`EMS` 负责把信号转成订单和成交链路，`Risk` 负责运行时风控，`Telegram` 提供用户 UI 和群消息入口。本文面向部署后的使用者，说明运行时风控规则和 Telegram 入口怎么用。

风控参数主要通过 Telegram UI 配置和查看，例如单笔金额、最大持仓、运行时间、止盈止损、当日最大亏损、连续亏损次数等。
