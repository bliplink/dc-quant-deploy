...
8. 容器健康门禁通过：ZooKeeper 使用 `ruok/imok`，A/B 同时检查服务端口和复制端口，GW 检查 TCP/WebSocket/HTTP 三端口；只有全部 healthy 后才写入 `last-successful.env`。

机器可读证据：

```text
/data/dc-saas-order-cluster-dev/evidence/20260907-144036/result.json
...
4. Docker 双实例、服务器资源限制、持久化目录、容器健康检查和一键回滚已完成隔离验证；故障检测与提升自动化仍需补齐。
